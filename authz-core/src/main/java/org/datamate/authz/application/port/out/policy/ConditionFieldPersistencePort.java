...
public interface ConditionFieldPersistencePort {
    ...
    ConditionField upsert(UUID id, UUID permissionId, String fieldName, FieldType fieldType,
    ...
    List<ConditionField> findActiveByPermissionId(UUID permissionId);

    /** Returns ALL fields (including DEPRECATED) for diff-sync comparison. */
    List<ConditionField> findAllByPermissionId(UUID permissionId);

    Optional<ConditionField> findByPermissionIdAndFieldName(UUID permissionId, String fieldName);

    /** Set field status to DEPRECATED (does not soft-delete). */
    void markDeprecated(UUID id);
